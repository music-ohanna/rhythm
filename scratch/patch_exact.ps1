...
if ($text.Contains($target1)) {
    ...
    Write-Host "1. Logo updated successfully"
} else {
    Write-Warning "1. Logo target not found"
}
...
if ($text.Contains($target2)) {
    ...
    Write-Host "2. Help button updated successfully"
} else {
    Write-Warning "2. Help button target not found"
}

# 3. Add newScreenDialog modal HTML after submissionDialog closing tag
...
"@
        ...
        Write-Host "3. newScreenDialog inserted successfully"
    }
}

# 4. tutorialSteps array replacement
...
"@
        $text = $text.Substring(0, $idx4Start) + $newTutorialSteps + $text.Substring($endPos)
        Write-Host "4. tutorialSteps updated successfully"
    }
}
...
Write-Host "All changes written to index.html successfully!"
