# Set PowerShell script encoding to UTF8
$filePath = Join-Path $PSScriptRoot "..\index.html"
$bytes = [System.IO.File]::ReadAllBytes($filePath)
$text = [System.Text.Encoding]::UTF8.GetString($bytes)

# Helper function
function ConvertFrom-B64($str) {
    return [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($str))
}

# Base64 string constants computed directly via UTF8
$b64_modalHtml = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(@"
...
$b64_logoTag = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes('onclick="openNewScreenModal()" class="text-xl md:text-2xl font-black text-slate-900 tracking-tight flex items-center cursor-pointer hover:opacity-80 transition-opacity" title="새 화면으로 이동 (작업 저장/초기화)">'))

$b64_newFns = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(@"
...
$b64_steps = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(@"
const tutorialSteps = [
            {
                title: '1단계: 음표·쉼표 넣기',
...
# Decode B64 strings
$modalHtml = ConvertFrom-B64 $b64_modalHtml
$logoTag = ConvertFrom-B64 $b64_logoTag
$newFns = ConvertFrom-B64 $b64_newFns
$steps = ConvertFrom-B64 $b64_steps

# 1. Logo replacement
$target1 = 'class="text-xl md:text-2xl font-black text-slate-900 tracking-tight flex items-center">'
if ($text.Contains($target1)) {
    $text = $text.Replace($target1, $logoTag)
...
if ($idx3 -gt 0) {
    ...
    if ($closeDialogIdx2 -gt 0) {
        ...
        $text = $text.Substring(0, $insertPos) + $modalHtml + $text.Substring($insertPos)
        Write-Host "3. newScreenDialog inserted"
    }
}
...
if ($idxFn -gt 0) {
    ...
    if ($idxFnEnd -gt 0) {
        $fnEndPos = $idxFnEnd + 1
        $text = $text.Substring(0, $idxFn) + $newFns + $text.Substring($fnEndPos)
...
if ($idx4Start -gt 0) {
    ...
    if ($idx4End -gt 0) {
        $endPos = $idx4End + 2
        $text = $text.Substring(0, $idx4Start) + $steps + $text.Substring($endPos)
        Write-Host "5. tutorialSteps updated"
    }
}
