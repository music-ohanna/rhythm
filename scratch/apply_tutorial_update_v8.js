...
// 1. 하단 저작권/출처 문구 정갈하게 개편
const oldFooterTarget = /제작:[\s\S]*?<\/p>/;
text = text.replace(
...
// 2. 하단 옵션 체크박스 지저분한 타원 테두리 제거 (notationOptionsGroup)
text = text.replace(
    'id="notationOptionsGroup" class="flex items-center gap-1 bg-slate-100/90 border border-slate-200/80 rounded-full px-2.5 py-1 text-xs font-bold text-slate-700 shadow-inner"',
    'id="notationOptionsGroup" class="flex items-center gap-2 text-xs font-bold text-slate-700"'
);

// 3. ➕ 마디 추가 버튼 위치 및 크기 철통 고정 (+ 아이콘 크기 text-4xl ~ text-5xl, 오른쪽 끝 고정)
text = text.replace(
    /<button id="canvasNextMeasureBtn"[\s\S]*?<\/button>/,
    '<button id="canvasNextMeasureBtn" type="button" onclick="switchMeasure(activeMeasureIndex + 1)" class="absolute right-3 top-1/2 -translate-y-1/2 z-40 text-blue-600 hover:text-blue-800 font-black text-4xl md:text-5xl cursor-pointer p-1 bg-transparent border-0 outline-none hover:scale-110 active:scale-95 transition-transform" title="마디 추가">➕</button>'
);
...
if (tStart > 0 && tEnd > tStart) {
    ...
    console.log('Update script v8 written and applied safely!');
} else {
