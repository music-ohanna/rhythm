...
Write-Host "File loaded. Searching targets..." -ForegroundColor Cyan
...
if ($stepIdx -ge 0) {
    ...
    Write-Host "Found tutorialSteps block (len: $($oldStepsBlock.Length))" -ForegroundColor Green
...
'@
    ...
    Write-Host "tutorialSteps NOT found" -ForegroundColor Red
}

# 2. help button -> guide button
...
if ($helpBtnIdx -ge 0) {
    ...
    Write-Host "Found help button block" -ForegroundColor Green
...
'@
    ...
    Write-Host "help button NOT found" -ForegroundColor Red
}
...
if ($modalIdx -ge 0) {
    $modalEnd = $contentLF.IndexOf('</div>', $modalIdx)
    # Find matching closing div for choice-card then choice-dialog
    # Or find line until <script>
    ...
    if ($scriptIdx -gt $modalIdx) {
        ...
        Write-Host "Found helpSimpleModal block" -ForegroundColor Green
        $contentLF = $contentLF.Replace($modalBlock, "")
    }
} else {
    Write-Host "helpSimpleModal NOT found" -ForegroundColor Red
}
...
if ($tieIdx -ge 0) {
    ...
    Write-Host "Found drawTieCurve block" -ForegroundColor Green
...
'@
    ...
    Write-Host "drawTieCurve NOT found" -ForegroundColor Red
}
...
if ($tempoIdx -ge 0) {
    ...
    Write-Host "Found setTempo block" -ForegroundColor Green
...
'@
    ...
    Write-Host "setTempo NOT found" -ForegroundColor Red
}
...
if ($practiceIdx -ge 0) {
    ...
    Write-Host "Found showPracticeExample block" -ForegroundColor Green
...
'@
    ...
    Write-Host "showPracticeExample NOT found" -ForegroundColor Red
}

# Write back with CRLF
$finalContent = $contentLF -replace "`n", "`r`n"
[System.IO.File]::WriteAllText($FilePath, $finalContent, [System.Text.Encoding]::UTF8)
Write-Host "Finished precise patch! Saved file." -ForegroundColor Green
